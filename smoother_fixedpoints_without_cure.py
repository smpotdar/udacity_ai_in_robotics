# -*- coding: utf-8 -*-
"""
Created on Fri Dec 04 15:50:27 2015
...
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001):
    ...
    while True:    
        for i in range(len(path)):
            ...
            x1=newpath[i][0]
            y1=newpath[i][1]
            newpath[i][0]=newpath[i][0]+weight_data*(path[i][0]-newpath[i][0])+weight_smooth*(newpath[(i+1)%len(path)][0]+newpath[(i-1)%len(path)][0]-2*newpath[i][0])
            newpath[i][1]=newpath[i][1]+weight_data*(path[i][1]-newpath[i][1])+weight_smooth*(newpath[(i+1)%len(path)][1]+newpath[(i-1)%len(path)][1]-2*newpath[i][1])
            x2=newpath[i][0]
            y2=newpath[i][1]
...
#newpath=smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001)
#plot_vector(newpath)
#solution_check(smooth(testpath1), answer1)
