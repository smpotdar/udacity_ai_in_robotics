...
def run_with_graphs(params, printflag = False):
    myrobot = robot()
    myrobot.set(0.0, 1.0, 0.0)
    speed = 1.0
    err = 0.0
...
def twiddle(tol = 0.001): #Make this tolerance bigger if you are timing out!
    num_par=3
    p=[0 for col in range(num_par)]
    dp=[1 for col in range(len(p))]
    best_err=run(p)
    while(sum(dp)>tol):
...
params=twiddle()
#print params
err=run_with_graphs(params,False)
