# -*- coding: utf-8 -*-
"""
Created on Fri Dec 04 17:06:25 2015
...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 04 16:19:32 2015
...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        r=radius        
        if(self.x>=r and self.y>=r and self.x<=3*r):
            cte=self.y-2*r
        elif(self.x>=r and self.y<r and self.x<=3*r):
            cte=-self.y
        elif(self.x<r):
            c_x=r
            c_y=r
            cte=sqrt(pow((self.x-c_x),2)+pow((self.y-c_y),2))-r
        else:
            c_x=3*r
            c_y=r
            cte=sqrt(pow((self.x-c_x),2)+pow((self.y-c_y),2))-r
